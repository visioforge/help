...
* **Control No Encontrado en la Lista de Importación:** Asegúrese de que el VisioForge Media Framework (versión ActiveX) fue instalado correctamente con privilegios administrativos. Intente reinstalar el framework. Registrar manualmente el control `.ocx` usando `regsvr32` (ejecutar desde un símbolo del sistema de Administrador) podría ser necesario en casos raros (ej., `regsvr32 "C:\Program Files\VisioForge\All-in-One Media Framework ActiveX\Redist\VisioForge_Media_Player.ocx"` - ajuste la ruta según sea necesario, o use `Redist\x64` para el control de 64 bits).
