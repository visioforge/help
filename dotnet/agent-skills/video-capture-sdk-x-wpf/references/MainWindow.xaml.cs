...
namespace Simple_Video_Capture
{
    ...
    public partial class MainWindow : IDisposable
    {
        ...
        {
            // WPF's Closing event does not await async-void handlers, so cleanup would
            // otherwise race with WPF tearing down the window (including VideoView1).
            // Cancel the first close, run cleanup, then close manually. Re-entry must be a no-op.
            if (_closingHandled)
            {
                return;
            }
