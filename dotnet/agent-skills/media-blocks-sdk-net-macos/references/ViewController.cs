...
public partial class ViewController : NSViewController
{
    ...
    private async Task StartAsync()
    {
        ...
        // Start -> Stop -> Start guard: tear down any previous pipeline + blocks first.
        if (_pipeline != null)
        {
            await StopAsync();
        }
        ...
        {
            ShowMessage("Unable to configure camera settings. Select a video source and format.");
            await StopAsync();
            return;
        }
        ...
        // audio source + renderer — only when an audio device/format AND an output were resolved.
        if (audioSourceSettings != null)
        {
            ...
            if (audioOutput != null)
            {
                ...
                _audioRenderer = new AudioRendererBlock(audioOutput) { IsSync = false };
...
/// <summary>
/// Custom Window delegate — stops the pipeline and tears the SDK down on close.
/// </summary>
public class CustomWindowDelegate : NSWindowDelegate
{
    ...
    public override bool WindowShouldClose(NSObject sender)
    {
        ...
        // WindowShouldClose return true. Tearing the SDK down under a live pipeline is what we
        // are avoiding here.
