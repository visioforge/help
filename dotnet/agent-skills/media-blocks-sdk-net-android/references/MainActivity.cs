...
namespace Simple_Video_Capture
{
    ...
    public class MainActivity : Activity
    {
        ...
        private async void btSwitchCam_Click(object sender, EventArgs e)
        {
            try
            {
                if (_pipeline == null || _cameras == null || _cameras.Length < 2 || _videoSource == null)
                {
                    return;
                }
        ...
        private async void btStartRecord_Click(object sender, EventArgs e)
        {
            try
            {
                if (!_isRecording)
                {
                    ...
                    moviesDir.Mkdirs();
                    _recordingFilename = Path.Combine(moviesDir.AbsolutePath, $"visioforge_{now:yyyyMMdd_HHmmss}.mp4");
