...
namespace Simple_Video_Capture
{
    ...
    public partial class Window1 : IDisposable
    {
        ...
        {
            Dispatcher.Invoke(() => { mmLog.AppendText(txt + Environment.NewLine); });
        }
        ...
        private void lbViewVideoTutorials_MouseLeftButtonDown(object sender, MouseButtonEventArgs e)
        {
            ...
            {
                Log("Error opening tutorial link: " + ex.Message);
            }
        }
        ...
        {
            // Stop the capture before disposing the engine so the output file is finalized cleanly.
            tmRecording?.Stop();
            VideoCapture1?.Stop();
            DestroyEngine();
            Dispose();
