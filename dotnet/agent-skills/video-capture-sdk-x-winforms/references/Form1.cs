namespace Computer_Vision_Demo
{
    ...
    public partial class Form1 : Form
    {
        ...
        /// <summary>
        /// Handles the FormClosing event — stops and disposes the engines and tears the SDK down.
        /// </summary>
        ...
        {
            // Auto
            if (string.IsNullOrWhiteSpace(edIPUrl.Text) || !Uri.TryCreate(edIPUrl.Text, UriKind.Absolute, out var uri))
            {
                MessageBox.Show("Enter a valid camera URL");
                return null;
            }
        ...
        private async Task ConfigureVideoCaptureAsync()
        {
            ...
            {
                // from IP camera
                var ipSource = await SelectIPCameraSourceAsync();
                if (ipSource == null)
                {
                    return;
                }

                VideoCapture1.Video_Source = ipSource;
            }
