...
public class AppDelegate : UIApplicationDelegate
{
    ...
    private async Task StopCamera()
    {
        ...
        {
            // Brief settle delay before handing the file to the Photos importer. StopAsync drains the
            // pipeline, but on iOS the just-closed MP4 file handle can still be racing the photo-library
            // import for a moment; ~1s avoids an occasional "file not found / incomplete" import.
            // Task.Delay (not Thread.Sleep): SDK rule forbids Thread.Sleep, and this runs on a captured
            // UI sync context where a blocking sleep would freeze the UI — Task.Delay yields.
    ...
    /// <summary>
    /// Will terminate — best-effort teardown so the live pipeline is stopped and the SDK is
    /// released before the process exits. WillTerminate is synchronous, so use the sync
    /// Stop/Dispose overloads (StopAsync may need to marshal back to this same thread → deadlock).
    /// </summary>
