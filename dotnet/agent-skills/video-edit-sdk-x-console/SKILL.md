...
**Fix**: confirm `InitSDKAsync` runs before any `VideoEditCoreX` constructor. Confirm the redist NuGet matches the build platform (`x64` redist for x64, `x86` redist for x86, both for AnyCPU; `Linux.x64` / `macOS` for cross-platform builds). Pin each redist to the newest version published for that package at or before your wrapper's release - the redists are built on their own cadence, so check nuget.org rather than assuming the wrapper's number exists for them, and never let a redist run ahead of the wrapper.
