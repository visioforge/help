...
- Options de réordonnancement d'images (désactivé par défaut : les images B amènent l'encodeur à produire des horodatages inutilisables par les sorties MPEG-TS et les enregistrements MP4)
- Encodage basé sur la qualité
- Contrôle de débit : `ABR` (par défaut) ou `CBR` ; en mode `ABR`, un plafond de débit de données moyenné sur une fenêtre glissante
...
Ou un débit moyen avec un plafond strict : cible de 5 Mbps, jamais plus de 7 Mbps moyennés sur
une seconde quelconque :
...
émule le CBR au moyen des limites de débit de données et le signale dans son propre journal
(`CBR is unsupported on your system, emulating with custom data rate limits`), au moyen de limites qui lui sont propres, dérivées de
`Bitrate`. La sortie encodée est alors quasi constante plutôt que constante.

Le contrôle de débit exige également le runtime GStreamer 1.28.6, désormais livré avec tous les
paquets Apple — macOS, Mac Catalyst et iOS.

`ForceHWUsage` est sans effet sur iOS : tous les encodeurs VideoToolbox y sont matériels, GStreamer
n'y enregistre donc aucun élément exclusivement matériel distinct et la demande est déjà satisfaite.
