...
Rate control also needs the GStreamer 1.28.6 runtime, which ships with the macOS and Mac Catalyst
packages. The iOS package still bundles 1.24.9, whose VideoToolbox encoder does not expose these
properties at all — the setting is discarded there with a warning rather than silently applied.
