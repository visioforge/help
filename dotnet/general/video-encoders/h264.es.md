...
- Opciones de reordenamiento de cuadros (desactivado de forma predeterminada: los fotogramas B hacen que el codificador emita marcas de tiempo que las salidas MPEG-TS y las grabaciones MP4 no pueden usar)
- Codificación basada en calidad
- Control de tasa: `ABR` (por defecto) o `CBR`; en modo `ABR`, un techo de tasa de datos promediado sobre una ventana deslizante
...
O una tasa media con un techo duro: objetivo de 5 Mbps, nunca más de 7 Mbps promediados sobre
cualquier segundo:
...
enabled`): la tasa constante ya es un techo. Los límites se aplican en modo `ABR`.
...
emula CBR mediante los límites de tasa de datos y lo indica en su propio registro
(`CBR is unsupported on your system, emulating with custom data rate limits`), usando límites propios derivados de `Bitrate`. La
salida codificada resulta entonces casi constante en lugar de constante.

El control de tasa también requiere el runtime de GStreamer 1.28.6, que ahora se incluye en todos
los paquetes de Apple: macOS, Mac Catalyst e iOS.

`ForceHWUsage` no tiene efecto en iOS: allí todos los codificadores VideoToolbox son por hardware,
por lo que GStreamer no registra ningún elemento exclusivo de hardware aparte y la petición ya está
satisfecha.
