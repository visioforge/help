...
El bloque de mezcla de canales ponderada mezcla los canales estéreo izquierdo y derecho con pesos ajustables independientemente. Perfecto para archivos de karaoke CD+G donde el canal izquierdo contiene música instrumental y el canal derecho contiene mezcla completa con voces.
...
Dirección del pin | Tipo de medio | Cantidad de pines
--- | :---: | :---:
Entrada | Audio sin comprimir (estéreo) | 1
Salida | Audio sin comprimir (mono o estéreo) | 1
...
// Crear bloque de mezcla de canales ponderada con pesos personalizados
...
// Ajustar pesos durante la reproducción
channelMix.LeftChannelWeight = 0.5f;   // Reducir música a 50%
channelMix.RightChannelWeight = 0.5f;  // Aumentar voces a 50%
```

#### Propiedades

* **LeftChannelWeight** (float): Peso para el canal izquierdo (predeterminado 0.5). Rango: 0.0 - 1.0
* **RightChannelWeight** (float): Peso para el canal derecho (predeterminado 0.5). Rango: 0.0 - 1.0
* **OutputStereo** (bool): Salida como estéreo con mono duplicado a ambos L+R (true, predeterminado) o mono (false)

#### Caso de Uso

Este bloque está diseñado específicamente para archivos de karaoke CD+G con codificación de audio dual-mono donde:

* Canal izquierdo: Pista de acompañamiento instrumental (solo música)
* Canal derecho: Mezcla completa incluyendo voces principales

Los usuarios pueden controlar el nivel de voz ajustando los pesos de los canales izquierdo y derecho. Por ejemplo, para reducir las voces, aumentar LeftChannelWeight y disminuir RightChannelWeight.
