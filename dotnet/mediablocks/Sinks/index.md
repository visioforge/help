...
The sink takes its destination either as a filename argument or as a `WAVSinkSettings` object, which is also the form a saved pipeline document carries. Sample format comes from the upstream `AudioConverterBlock` settings.
...
For an end-to-end workflow — extracting KLV from an MPEG-TS file, decoding MISB ST 0601 elements, and muxing KLV back into MPEG-TS — see the [KLV / MISB Metadata in MPEG-TS](../Guides/klv-metadata-mpegts-csharp.md) guide.
